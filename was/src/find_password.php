...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT password FROM members WHERE username=? AND name=?");
    ...
    if ($result->num_rows > 0) {
        ...
        $success = "회원님의 비밀번호는: <strong>{$row['password']}</strong> 입니다.";
    } else {
        $error = "정보가 일치하지 않습니다.";
    }
}
...
<head>
<meta charset="UTF-8">
<title>비밀번호 찾기 - 보드게임 SOL드아웃</title>
